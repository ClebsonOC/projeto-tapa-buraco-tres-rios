...
router.get("/buscar-bairros", async (req, res) => {
    try {
        res.json(await getBairrosComCache()); // <-- Chamada atualizada
    } catch (error) {
...
router.delete("/buracos/:docId", async (req, res) => {
    ...
    try {
        await firestore.runTransaction(async (transaction) => {
            ...
            if (dataRegistro.getTime() !== hoje.getTime()) throw new Error("Ação não permitida. Registros só podem ser deletados no mesmo dia da criação.");
            ...
            if (visitaSnapshot.docs.length <= 1) throw new Error("Não é possível deletar o último buraco de uma visita. Para isso, delete a visita inteira.");
        ...
        if (error.message.includes("Ação não permitida")) return res.status(403).json({ error: error.message });
        if (error.message.includes("Não é possível deletar o último buraco")) return res.status(400).json({ error: error.message });
...
router.patch("/buracos/dimensoes/:docId", async (req, res) => {
    try {
        ...
        if (dataRegistro < limiteEdicao) {
            return res.status(403).json({ error: "Ação não permitida. Registros só podem ser alterados em até 10 dias." });
        }
...
router.post("/efetivo", async (req, res) => {
    try {
        ...
        const snapshot = await firestore.collection('efetivo').where('registradoPor', '==', registradoPor).where('registradoEm', '>=', inicioDoDia).where('registradoEm', '<=', fimDoDia).get();
        if (!snapshot.empty) return res.status(409).json({ error: "Já existe um registro de efetivo para este usuário hoje." });
        const novoEfetivo = { registradoPor, registradoEm: new Date(), itensPresentes, observacao: observacao || "" };
